...
function OperatingButtons(props) {
    const { dealCards, disabledDeal, showCards, disabledShowdown, cleanAfterShowdown } = props
    return (
        <div className='operatingButtons'>
            ...
            {disabledDeal ?
                <button onClick={cleanAfterShowdown} disabled={disabledShowdown}>New hand</button> :
                <button onClick={cleanAfterShowdown} disabled={disabledDeal}>New Game</button>
            }
        </div>
    )
}
